...
class IndexController extends Controller
{
    ...
    protected $viewPath = 'jiny-post::www.board';
    ...
    public function __invoke(Request $request, $code)
    {
        ...
        // JWT 인증 디버깅 (BoardPost IndexController)
        \Log::info('BoardPost IndexController Debug', [
        ...
        if ($request->has('search') && $request->search) {
            ...
            foreach ($paginatedItems as $item) {
                ...
                if (Schema::hasTable($imageTable)) {
                    $imageCounts[$item->id] = DB::table($imageTable)
                ...
                if (Schema::hasTable($attachmentTable)) {
                    $attachmentCounts[$item->id] = DB::table($attachmentTable)
                        ->where('post_id', $item->id)
                        ->count();
                } else {

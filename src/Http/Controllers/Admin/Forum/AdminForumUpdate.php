 ...
 */
class AdminForumUpdate extends Controller
{
    /**
     * 데이터베이스 테이블명
     *
     ...
     * @param int|string $id 수정할 포럼 글의 ID
     ...
     * {
     *   "success": true,
     *   "message": "포럼 글이 수정되었습니다.",
     *   "data": {
     *     "id": 123,
     *     "title": "수정된 제목",
     *     "updated_at": "2024-10-22T15:30:00.000000Z"
     *   }
     * }
     */
    public function __invoke(Request $request, $id)
    {
        try {
             ...
             * - remove_image_ids: 삭제할 이미지 ID 목록 (콤마로 구분)
             */
            ...
            // CSRF 토큰과 HTTP 메서드 필드, 이미지 관련 필드 제외하고 데이터 준비
            $data = $request->except(['_token', '_method', 'images', 'remove_image_ids']);
            ...
            try {
                 ...
                 * 다중 이미지 처리 로직 디버깅
                 */
                \Log::info('Admin forum update debug info:', [
                    'forum_id' => $id,
                ...
                if ($request->filled('remove_image_ids')) {
                    ...
                    if (!empty($removedImageIds)) {
                        \Log::info('Removing existing images:', ['ids' => $removedImageIds]);
             ...
             */
            if ($request->ajax() || $request->expectsJson() || $request->wantsJson()) {
                // AJAX 요청: JSON 응답 반환 (HTTP 200 OK)
                return response()->json([
                    'success' => true,
                    'message' => '포럼 글이 수정되었습니다.',
                    'data' => [
            ...
            return redirect()->back()
                ->withInput()
                ->with('error', '포럼 글 수정 중 오류가 발생했습니다.');
        }
    }
     ...
     * 포럼 이미지 삭제 처리
     *
     * @param array $imageIds 삭제할 이미지 ID 배열
     * @return void
     */
    protected function removeForumImages($imageIds)
    {
        ...
        foreach ($images as $image) {
            try {
                ...
                \Log::error('Failed to delete admin forum image: ' . $e->getMessage(), [
                    'image_id' => $image->id,

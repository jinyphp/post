...
<div class="container py-5">
    <div class="row">
        <div class="col-12" style="padding-left: 0; padding-right: 0;">
            ...
            <!-- 첨부 이미지 -->
            <!-- Debug: Images count = {{ count($images) }} -->
            @if(count($images) > 0)
                <div class="mb-4">
                    <h6 class="mb-3">
                        <i class="bi bi-images text-primary"></i> 첨부 이미지 ({{ count($images) }}개)
                    </h6>
                    <!-- Debug: First image path = {{ isset($images[0]) ? $images[0]->file_path : 'N/A' }} -->
                    <div class="row g-3">
                            ...
                            <div class="col-md-6 col-lg-4">
                                <div class="card border-0 shadow-sm">
                                    <img src="{{ asset('storage/' . $image->file_path) }}"
                                         ...
                                         onclick="showImageModal('{{ asset('storage/' . $image->file_path) }}', '{{ $image->original_name }}')">
            ...
            @if(count($attachments) > 0)
                <div class="mb-4">
                    <h6 class="mb-3">
                        <i class="bi bi-paperclip text-success"></i> 첨부파일 ({{ count($attachments) }}개)
                    </h6>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        ...
                                        <small class="text-muted">
                                            ...
                                            {{ \Carbon\Carbon::parse($attachment->created_at)->format('Y-m-d H:i') }}
                                        </small>
                                        @if($attachment->download_count > 0)
                                            <small class="text-muted d-block">
                                                <i class="bi bi-download"></i> {{ number_format($attachment->download_count) }}회 다운로드
                                            </small>
                                        @endif
                                    </div>
                                </div>
                                <a href="{{ route('board.download', [$code, $post->id, $attachment->id]) }}"
            ...
            <!-- Livewire 댓글 컴포넌트 -->
            @livewire('jiny-post::board-comment', [
                'code' => $code,
                'postId' => $post->id
            ])
